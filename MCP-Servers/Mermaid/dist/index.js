...
function isGenerateArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "code" in args &&
        typeof args.code === "string" &&
        (!args.theme ||
            ["default", "forest", "dark", "neutral"].includes(args.theme)) &&
        (!args.backgroundColor ||
            typeof args.backgroundColor === "string") &&
        (!args.outputFormat ||
            ["png", "svg"].includes(args.outputFormat)) &&
        (!args.name || typeof args.name === "string") &&
        (!args.folder || typeof args.folder === "string"));
}
async function renderMermaid(code, config = {}) {
    ...
    `;
        ...
        log(LogLevel.ERROR, `Error in renderMermaid: ${error instanceof Error ? error.message : String(error)}`);
 ...
 */
async function saveMermaidImageToFile(base64Image, name, folder) {
 ...
 * @returns The full path to the saved file
 */
async function saveMermaidSvgToFile(svgContent, name, folder) {
    // Create the folder if it doesn't exist
    if (!fs.existsSync(folder)) {
        log(LogLevel.INFO, `Creating folder: ${folder}`);
        fs.mkdirSync(folder, { recursive: true });
    ...
    try {
        ...
        if (!CONTENT_IMAGE_SUPPORTED) {
            ...
            if (outputFormat === "svg") {
                ...
                fullPath = await saveMermaidSvgToFile(result.svg, args.name, args.folder);
            }
            else {
                fullPath = await saveMermaidImageToFile(result.data, args.name, args.folder);
            }
            return {
                content: [
                    {
        ...
        if (args.folder && args.name) {
            try {
                ...
                log(LogLevel.ERROR, `Failed to save ${outputFormat} to folder: ${saveError.message}`);
                savedMessage = `Note: Failed to save ${outputFormat} to folder: ${saveError.message}`;
            }
        }
