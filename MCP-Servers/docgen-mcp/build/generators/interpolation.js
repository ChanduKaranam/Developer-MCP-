export async function interpolateTemplate(template, data, projectId) {
...
function replaceBasicVariables(content, projectId) {
...
async function processSectionContent(content, section, data) {
...
async function generateSectionContent(section, data) {
...
function generateObjectiveSection(data) {
...
function generateMaterialsSection(data) {
...
function generateMethodsSection(data) {
    ...
    if (scriptSources.length > 0) {
        for (const [_, info] of scriptSources) {
            ...
            if (script.workflow && script.workflow.length > 0) {
                methods.push(...script.workflow.map((step, index) => `${index + 1}. ${step}`));
            }
        }
    }
    return methods.join('\n');
...
function generateResultsSection(data) {
    ...
    if (presentationSources.length > 0) {
        ...
        if (presentation.extractedInfo && presentation.extractedInfo.key_findings) {
            results.push(...presentation.extractedInfo.key_findings.map((finding) => `- ${finding}`));
        }
    }
    return results.join('\n');
}
